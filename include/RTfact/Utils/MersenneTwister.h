...
public:
	...
	double rand(const double& l, const double& h);
	uint32 randInt();                       // integer in [0,2^32-1]
	uint32 randInt( const uint32& n );      // integer in [0,n] for n < 2^32
	uint32 randInt(int l, int h);
...
inline double MTRand::rand(const double& l, const double& h)
	{ return rand() * (h - l) + l; }
...
inline MTRand::uint32 MTRand::randInt(int l, int h)
{
	return (MTRand::uint32)(rand(0.0, h - l + 1.0) + l);
}
