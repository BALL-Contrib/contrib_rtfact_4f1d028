 ...
 */
 ...
 #ifndef RTFACT__BVHINTERSECTOR_HPP
#define RTFACT__BVHINTERSECTOR_HPP
...
protected:
    ...
    template<
        class tIterator>
    struct TraversalState<1, tIterator>
    {
        float     tMin;
        bool      isLeaf;
        tIterator firstChildIterator;
...
public:
    ...
    //using typename tIntersector::Result;
        ...
        typename tIntersector::template Result<taSize>& oResult) const
    {
        ...
        TraversalState<taSize, typename tBVH::Iterator>
            traversalStack[RTFACT_TRAVERSER_MAX_STACK_DEPTH];
        int stackTop = -1;

        const typename tBVH::Iterator rootIterator = aTree.getRoot();

        bool isLeaf = aTree.isLeaf(rootIterator);
        typename tBVH::Iterator firstChildIterator, secondChildIterator;
        ...
        aTree.getChildren(rootIterator,
            firstChildIterator, secondChildIterator);

        for(;;)
        {
            while(!isLeaf && firstActiveRayId < taSize)
            {
                ActiveRay firstActiveRaySecondChild;
                firstActiveRaySecondChild.org    = firstActiveRay.org;
                firstActiveRaySecondChild.invDir = firstActiveRay.invDir;

                //check second child
                const uint firstActiveRayIdSecondChild =
                    findFirstActiveRay<taCommonOrg>(
                        aRayPacket, aRayMask, aRayPacket.tMin, oResult.dist,
                        firstActiveRayId, firstActiveRaySecondChild,
                        aTree.getBounds(secondChildIterator));
                ...
                firstActiveRayId =
                    findFirstActiveRay<taCommonOrg>(
                        aRayPacket, aRayMask, aRayPacket.tMin, oResult.dist,
                        firstActiveRayId, firstActiveRay,
                        aTree.getBounds(firstChildIterator));

                //INCREMENT_COUNTER(0);
                //INCREMENT_COUNTER(0);

                const bool enterFirstChild = (firstActiveRayId < taSize);
                const bool enterSecondChild = (firstActiveRayIdSecondChild < taSize);

                if(enterFirstChild && enterSecondChild)
                {
                    // check traversal order
                    if(firstActiveRay.boxTMin < firstActiveRaySecondChild.boxTMin)
                    {
                        //push second child on the traversalStack
                        traversalStack[++stackTop].firstActiveRayId = firstActiveRayIdSecondChild;
                        traversalStack[stackTop].isLeaf = aTree.isLeaf(secondChildIterator);

                        aTree.getChildren(secondChildIterator,
                        ...
                        aTree.getChildren(firstChildIterator,
                                          firstChildIterator, secondChildIterator);
                    }
                    else
                    {
                        //push second first on traversalStack
                        ...
                        firstActiveRayId = firstActiveRayIdSecondChild;
                        firstActiveRay.invDir = firstActiveRaySecondChild.invDir;

                        if(!taCommonOrg)
                        {
                            firstActiveRay.org = firstActiveRaySecondChild.org;
                        }
                    }
                }
                else if(enterFirstChild)
                {
                    isLeaf = aTree.isLeaf(firstChildIterator);
            ...
            if(firstActiveRayId < taSize)
            {
                ...
                aTree.getElements(firstChildIterator, secondChildIterator,
...
#endif // RTFACT__BVHINTERSECTOR_HPP
